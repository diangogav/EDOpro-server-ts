...
describe("WaitingState", () => {
	...
	beforeEach(() => {
		...
		state = new WaitingState(mockEmitter, mockLogger, mockUserAuth, mockDeckCreator);
	});
	...
	it("should handle TO_DUEL command (spectator to player)", () => {
		...
		mockEmitter.emit(Commands.TO_DUEL as unknown as string, message, mockRoom, mockClient);
	...
	it("should handle TO_DUEL command (move cell)", () => {
		...
		expect(mockRoom.movePlayerToAnotherCellUnsafe).toHaveBeenCalledWith(mockClient);
	});
	...
	it("should handle NOT_READY command", () => {
		...
		mockEmitter.emit(Commands.NOT_READY as unknown as string, message, mockRoom, mockClient);
	...
	it("should handle OBSERVER command", () => {
		...
		mockEmitter.emit(Commands.OBSERVER as unknown as string, message, mockRoom, mockClient);
	...
	it("should handle TRY_START command", () => {
		...
		mockEmitter.emit(Commands.TRY_START as unknown as string, message, mockRoom, mockClient);
	...
	it("should handle JOIN event (success)", async () => {
		...
		mockRoom.calculatePlaceUnsafe = jest.fn().mockReturnValue({ position: 0, team: 0 });
