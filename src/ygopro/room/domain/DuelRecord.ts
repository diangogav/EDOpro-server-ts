...
export class DuelRecord {
	...
	toYrp(room: Pick<YGOProRoom, "hostInfo" | "isTag">) {
		...
		const yrp = new YGOProYrp({
			...
			clientDeck: isTag ? this.toReplayDeck(players[2]?.deck) : this.toReplayDeck(players[1]?.deck),
	...
	*toPlayback(
	...
	private resolveObserverWinMsg() {
		if ((this.winPosition !== 0 && this.winPosition !== 1) || typeof this.winReason !== "number") {
			return undefined;
		}
